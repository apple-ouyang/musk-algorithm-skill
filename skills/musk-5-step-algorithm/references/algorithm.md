# The Algorithm

## Canonical sequence

The corpus supports this ordered sequence as the public core of Musk's five-step method:

1. Make the requirements less dumb.
2. Try very hard to delete the part or process.
3. Simplify and optimize the design.
4. Accelerate cycle time.
5. Automate.

## Interpretation

### Step 1

Treat every requirement as a hypothesis, not gospel. Push for named ownership rather than vague department-level authority.

### Step 2

Deletion is a feature, not a cleanup pass. Removing steps exposes what the system actually needs.

### Step 3

Optimization is allowed only after unnecessary parts have been removed. Otherwise the model is polishing waste.

### Step 4

Speed matters after the structure is correct. Faster iteration on a bad process is still bad process.

### Step 5

Automation amplifies the process that exists. If the underlying workflow is dumb, automation industrializes the dumbness.
